...
export default function ReliabilityObjTypeTestUpdateForm(props) {
  const {
    id,
  ...
  const resetStateValues = () => {
    const cleanValues = { ...initialValues, ...reliabilityObjTypeTestRecord };
    setQuestion_1(cleanValues.question_1);
    setQuestion_2(cleanValues.question_2);
    setQuestion_3(cleanValues.question_3);
    setQuestion_4(cleanValues.question_4);
    setQuestion_5(cleanValues.question_5);
    setQuestion_6(cleanValues.question_6);
    setQuestion_7(cleanValues.question_7);
    setQuestion_8(cleanValues.question_8);
    setQuestion_9(cleanValues.question_9);
    setQuestion_10(cleanValues.question_10);
    setErrors({});
  };
  const [reliabilityObjTypeTestRecord, setReliabilityObjTypeTestRecord] =
    React.useState(reliabilityObjTypeTest);
  React.useEffect(() => {
    const queryData = async () => {
      const record = id
        ? await DataStore.query(ReliabilityObjTypeTest, id)
        : reliabilityObjTypeTest;
      setReliabilityObjTypeTestRecord(record);
    };
    queryData();
  }, [id, reliabilityObjTypeTest]);
  React.useEffect(resetStateValues, [reliabilityObjTypeTestRecord]);
  const validations = {
    question_1: [],
    question_2: [],
    question_3: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          await DataStore.save(
            ReliabilityObjTypeTest.copyOf(
              reliabilityObjTypeTestRecord,
              (updated) => {
      ...
      {...rest}
      {...getOverrideProps(overrides, "ReliabilityObjTypeTestUpdateForm")}
    >
      <TextField
        label="Question 1"
        isRequired={false}
