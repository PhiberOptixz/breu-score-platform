...
export default function CandidateBelievabilityCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
  ...
  const resetStateValues = () => {
    ...
    setTotal_years_of_exp(initialValues.total_years_of_exp);
    setPrefered_programming_lang(initialValues.prefered_programming_lang);
    setFramework_proficiency(initialValues.framework_proficiency);
    setEducation(initialValues.education);
    setDomain(initialValues.domain);
    setGithub(initialValues.github);
    setStackoverflow(initialValues.stackoverflow);
    setLinkedin(initialValues.linkedin);
    setBlogs(initialValues.blogs);
    setErrors({});
  };
  const validations = {
    role: [{ type: "Required" }],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          await DataStore.save(new CandidateBelievability(modelFields));
          if (onSuccess) {
      ...
      {...rest}
      {...getOverrideProps(overrides, "CandidateBelievabilityCreateForm")}
    >
      <TextField
        label="Role"
        isRequired={true}
